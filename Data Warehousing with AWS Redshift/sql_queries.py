...
staging_events_copy = ("""
copy staging_events_table (artist, auth, firstName, gender,
...
JOIN (SELECT s.song_id AS song_id, a.artist_id AS artist_id, s.title AS song, a.name AS artist, s.duration AS length 
FROM songs s
JOIN artists a ON s.artist_id=a.artist_id) sa ON se.song=sa.song AND se.artist=sa.artist AND se.length=sa.length; 
""")
...
FROM (SELECT userId, firstName, lastName, gender, level,
   ...
   WHERE userId IS NOT NULL) AS ranked
...
FROM (SELECT song_id, title, artist_id, year, duration,
   ...
   WHERE song_id IS NOT NULL) AS ranked
...
FROM (SELECT artist_id, artist_name, artist_location, artist_latitude, artist_longitude,
   ...
   FROM staging_songs_table
   WHERE artist_id IS NOT NULL) AS ranked
